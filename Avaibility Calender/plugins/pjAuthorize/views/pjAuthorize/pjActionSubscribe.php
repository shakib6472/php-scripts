...
(function($, undefined) {
	...
	AuthorizeNetPopup.onReceiveCommunication = function (querystr) {
		...
		case "transactResponse":
			...
			if (parseInt(response.responseCode, 10) === 1) {
    			$.post("<?php echo PJ_INSTALL_URL; ?>index.php?controller=pjAuthorize&action=pjActionGetCustomer", {
    				transId: response.transId,
    				amount: response.totalAmount,
    				description: response.orderDescription,
    				custom: response.orderInvoiceNumber
    			}).done(function(data) {
