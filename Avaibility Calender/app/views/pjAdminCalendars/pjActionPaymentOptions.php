...
$foreign_id = $controller->_get->toInt('calendar_id');
$active_arr = array();
if(pjObject::getPlugin('pjPayments') !== NULL)
{
    $active_arr = pjPayments::getActivePaymentMethods($foreign_id);
}
?>
<form id="frmPaymentOptions" action="?" method="post" class="form-horizontal">
	...
	<div class="modal-body">
		...
		if(!in_array($payment_method, array('bank', 'cash')))
		{
			...
			{
			    $controller->requestAction(array('controller' => $pjPlugin, 'action' => 'pjActionOptions', 'params' => array('foreign_id' => $foreign_id, 'fid' => $foreign_id)));
			}
		}else{
		    ?>
